...
export default function DepartmentsIndex() {
  ...
  useEffect(() => {
    const fetchDepartments = async () => {
      try {
        setLoading(true);
        const res = await axios.get('/departments/list', { params: { page: pageNum, per_page: perPage, search } });
        ...
        <div className="flex justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              ...
              className="px-3 py-2 text-sm font-medium text-white bg-slate-600 rounded-md hover:bg-slate-700 inline-flex items-center gap-2"
            >
              <BoxArchiveSolidFullIcon className="size-4 text-white fill-white" />
              {t('departments.archived_title')}
            </Link>
            <Link
